...
public class Encoder {
  ...
  public String encode(String unencodedText) {
    ...
    for (int encodedWordsCounter = 0; encodedWordsCounter < splittedUnencodedText.length;
        encodedWordsCounter++) {
  ...
  public int findOccurencesOfSequenceInString(String needle, String haystack) {
    ...
    while (currentOccurencePosition != -1) {
      occurenceCounter++;
      currentOccurencePosition = haystack
          .indexOf(needle, currentOccurencePosition + needle.length());
    }
  ...
  private void handleIncompletelySeparatedWords(String wordToEncode) {
    if (findOccurencesOfSequenceInString("]", wordToEncode) == findOccurencesOfSequenceInString("[",
      ...
      for (int amountOfSpecialWordsEncoded = 0;
          amountOfSpecialWordsEncoded < findOccurencesOfSequenceInString("]", wordToEncode);
          amountOfSpecialWordsEncoded++) {
